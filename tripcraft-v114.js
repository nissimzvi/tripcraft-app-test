/* TripCraft Mobile V114 - iPhone-safe account actions and logo-only navigation. */
(() => {
  'use strict';
  ...
  async function cloudRecords() {
    ...
    const readTable = async (table, source) => {
      try {
        ...
        console.warn(`V114: ${table} unavailable`, error?.message || error);
        return [];
      }
    };
  ...
  function accountCard(record) {
    ...
    return `<article class="safe tc-v112-trip-card" data-v112-card="${escapeHtml(record.id)}">
      ...
      <div class="cta tc-v112-trip-actions">
        <button class="btn secondary" type="button" data-v114-action="open" data-v112-open="${escapeHtml(record.id)}">${t('פתח ועדכן טיול', 'Open and Update Trip')}</button>
        <button class="btn green" type="button" data-v114-action="copy" data-v112-copy="${escapeHtml(record.id)}">${t('העתק קישור', 'Copy Link')}</button>
        <button class="btn tc-v112-delete" type="button" data-v114-action="delete" data-v112-delete="${escapeHtml(record.id)}">${t('מחק טיול', 'Delete Trip')}</button>
      </div>
    </article>`;
  }
  ...
  function renderAccountBody(list) {
    ...
    if (!box) return;
    if (!user) {
      box.innerHTML = `<div class="safe tc-v114-account-view">${t('כדי לראות את הטיולים יש להיכנס עם אימייל וקוד אימות.', 'Sign in with your email and verification code to view your trips.')}</div>`;
      return;
    }
    const name = [user.firstName || user.first_name, user.lastName || user.last_name].filter(Boolean).join(' ') || t('החשבון שלי', 'My Account');
    const notice = Number(sessionStorage.getItem('tc_v114_saved_notice') || 0);
    ...
    box.dataset.v114Rendered = '1';
    box.innerHTML = `${saved}<section class="account-card tc-v112-account-card tc-v114-account-card tc-v114-account-view">
      <h3>${escapeHtml(name)}</h3>
      <p>${escapeHtml(user.email || '')}${user.phone ? ` · ${escapeHtml(user.phone)}` : ''}</p>
      <div class="cta"><button class="btn planner-color" id="tcV114NewTrip" data-v114-action="new" type="button">${t('בניית טיול חדש', 'Build a New Trip')}</button></div>
    </section>
  ...
  function bindAccountActions(box) {
    box.querySelectorAll('[data-v114-action]').forEach(button => {
  ...
  async function activateAccountAction(button) {
    ...
    const action = button.dataset.v114Action;
    if (action === 'new') return startNewTrip();
    if (action === 'copy') { copyText(actionUrl(button.dataset.v112Copy), button); return true; }
    if (action === 'delete') { deleteTrip(button.dataset.v112Delete); return true; }
    if (action !== 'open' || button.dataset.v114Busy === '1') return false;
    button.dataset.v114Busy = '1';
    button.disabled = true;
    const old = button.textContent;
    button.textContent = t('פותח...', 'Opening...');
    try { await openTrip(button.dataset.v112Open); return true; }
    ...
    if (sb && user) {
      for (const table of ['tripcraft_trips', 'trips']) {
        try {
          ...
          if (error) throw error;
        } catch (error) { console.warn(`V114: delete from ${table} skipped`, error?.message || error); }
      }
    }
  ...
  async function saveCurrentTrip(button) {
    ...
    try {
      ...
      sessionStorage.setItem('tc_v114_saved_notice', String(Date.now()));
      await renderAccount();
  ...
  function handleClick(event) {
    const accountAction = event.target.closest?.('[data-v114-action]');
    if (accountAction) {
      stop(event);
  ...
  function install() {
    ...
    ensureCalendar();
    enhancePlanner();
    window.TripCraftV114 = { VERSION, renderAccount, openTrip, startNewTrip, saveCurrentTrip, deleteTrip, activateAccountAction };
    ...
    new MutationObserver(() => {
      ...
      requestAnimationFrame(() => {
        ...
        if (location.hash === '#account' && account && !account.querySelector('.tc-v114-account-view') && !accountRepairQueued) {
    ...
    document.documentElement.dataset.tripcraftBuild = 'V114-MOBILE';
  }
