...
(() => {
  ...
  function handleCurrentRoute(){
    ...
    const page=raw.split('?')[0]||'home';if(PROTECTED.has(page)&&!loggedIn()){goLogin('#'+page);return}if(page==='account'){if(window.TripCraftV115)window.TripCraftV115.renderAccount();else setTimeout(()=>window.TripCraftV115?.renderAccount?.(),120)}
  }
  function installRouteGate(){
    document.addEventListener('click',e=>{
      ...
      // V107: a valid persisted Supabase session remains usable throughout the app.
      ...
      if(page==='planner'&&loggedIn()){e.preventDefault();e.stopImmediatePropagation();if(cloudTrips.length){location.hash='#account';setTimeout(()=>window.TripCraftV115?.renderAccount?.(),0);}else startNewTrip();}
    },true);
  ...
  async function init(){
    ...
    await syncSession();
    window.tcRenderAccount=(...args)=>window.TripCraftV115?.renderAccount?.(...args);try{tcRenderAccount=window.tcRenderAccount}catch(e){}
