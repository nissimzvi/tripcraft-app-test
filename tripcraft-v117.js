/* TripCraft Mobile V117 - kosher lodging search and reliable final actions. */
(() => {
  'use strict';
  ...
  function syncFloatingActions() {
    ...
    const finalStep = Boolean(document.querySelector('.planner-step.active[data-step="7"]')) || window.planner?.step === 7;
    ...
    if (prev) prev.style.display = resultVisible ? 'none' : 'inline-flex';
    if (next) next.style.display = resultVisible || finalStep ? 'none' : 'inline-flex';
    if (build) build.style.display = !resultVisible && finalStep ? 'inline-flex' : 'none';
  }
  ...
  function install() {
    ...
    document.documentElement.dataset.tripcraftBuild = 'V117-MOBILE';
  }
