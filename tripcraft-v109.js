/* TripCraft V109 - persistent authenticated navigation and reliable trip editing. */
(() => {
  'use strict';
  ...
  function installLogo(){
    document.querySelectorAll('.tc-brand-logo,img[src*="tripcraft-logo.png"]').forEach(img=>{
      img.src='tripcraft-logo-v109.png?v=109-final';
      img.removeAttribute('srcset');
      img.style.display='block';
      img.style.visibility='visible';
      img.style.opacity='1';
      const link=img.closest('a');
      if(link){link.href=hasUsableLocalSession()?'#account':'#home';link.dataset.tcHomeLogo='1'}
    });
  }
  ...
  function handleInternalLink(event,link){
    const href=link.dataset.tcHomeLogo==='1'?(hasUsableLocalSession()?'#account':'#home'):link.getAttribute('href');
  ...
  async function handleBuild(event,button){
    ...
    try{
      ...
      console.error('TripCraft V109 build failed',error);
  ...
  function delegatedActions(event){
    ...
    if(logo){event.preventDefault();event.stopImmediatePropagation();closeMobileMenu();const target=hasUsableLocalSession()?'#account':'#home';if(window.TripCraftV108State?.shouldGuardNavigation?.(target))window.TripCraftV108State.confirmNavigation(target);else routeNow(target);return}
...
/* TripCraft V109 - room-aware hotels, future-date rules, explicit save and safe deletion. */
(() => {
  'use strict';
  ...
  function installLogoGuarantee(){
    document.querySelectorAll('.tc-brand-logo').forEach(img=>{
      img.src='tripcraft-logo-v109.png?v=109-final';img.style.display='block';img.style.visibility='visible';img.style.opacity='1';
      img.onerror=()=>{img.onerror=null;img.src='icons/tripcraft-header-logo.png?v=109-final'};
      const link=img.closest('a');if(link){link.dataset.tcHomeLogo='1'}
    });
  }
  ...
  function install(){
    const style=document.createElement('style');style.id='tc-v108-feature-styles';style.textContent=`
      ...
      @media(max-width:620px){.tc-brand-logo{width:78px!important;height:62px!important;min-width:78px!important}.tc-v108-stars{grid-template-columns:1fr 1fr!important}.tc-v108-result-actions{display:grid;width:100%;grid-template-columns:1fr}.tc-v108-result-actions .btn{width:100%}#tcAccountBody .v102-trip-card>.cta{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:6px!important}#tcAccountBody .v102-trip-card>.cta .btn{width:100%!important;min-width:0!important;padding:9px 5px!important;font-size:12px!important;white-space:normal!important}}
    `;document.head.appendChild(style);
    ...
    document.documentElement.dataset.tripcraftVersion='V109';
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
})();

/* V109 - itinerary-aware lodging plan, Booking affiliate routing and public demo. */
(() => {
  'use strict';
