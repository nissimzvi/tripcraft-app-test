/* TripCraft V89 auth + cloud trips flow.
...
(() => {
  ...
  async function tcAuthRequest(endpoint,payload){
    ...
    try{
      const res=await fetch(url,{method:'POST',mode:'cors',cache:'no-store',credentials:'omit',signal:ctl.signal,headers:{'Content-Type':'application/json','apikey':c.supabaseAnonKey,'X-Client-Info':'tripcraft-v89'},body:JSON.stringify(payload||{})});
  ...
  function authReturn(hash){sessionStorage.setItem('tc_v89_return_after_auth',hash||'#account')}
  ...
  async function v85VerifyOtp(e){
    ...
    const token=String(byId('tcOtpCode')?.value||'').replace(/\D/g,'');if(token.length<6||token.length>8){byId('tcOtpStatus').textContent='נא להזין את קוד האימות שקיבלתם במייל.';byId('tcOtpCode')?.focus();return}
    ...
    try{
      ...
      const ret=sessionStorage.getItem('tc_v89_return_after_auth')||'#account';sessionStorage.removeItem('tc_v89_return_after_auth');location.hash=ret;
