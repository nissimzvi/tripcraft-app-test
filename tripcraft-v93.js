...
(() => {
  ...
  async function tcAuthRequest(endpoint,payload){
    ...
    try{
      const res=await fetch(url,{method:'POST',mode:'cors',cache:'no-store',credentials:'omit',signal:ctl.signal,headers:{'Content-Type':'application/json','apikey':c.supabaseAnonKey,'X-Client-Info':'tripcraft-v93'},body:JSON.stringify(payload||{})});
  ...
  function authReturn(hash){sessionStorage.setItem('tc_v93_return_after_auth',hash||'#account')}
  ...
  function installAuthUI(){
    ...
    if(otp){
      ...
      byId('tcOtpCode')?.addEventListener('paste',e=>{e.preventDefault();const digits=String(e.clipboardData?.getData('text')||'').replace(/\D/g,'').slice(0,8);e.target.value=digits;byId('tcOtpStatus').textContent='';requestAnimationFrame(()=>{e.target.focus();try{e.target.setSelectionRange(digits.length,digits.length)}catch(_){}});});
  ...
  async function v85VerifyOtp(e){
    ...
    try{
      ...
      const ret=sessionStorage.getItem('tc_v93_return_after_auth')||'#account';sessionStorage.removeItem('tc_v93_return_after_auth');location.hash=ret;
