/* TripCraft V85 auth + cloud trips flow.
...
(() => {
  ...
  function authReturn(hash){sessionStorage.setItem('tc_v85_return_after_auth',hash||'#account')}
  ...
  async function v85CheckEmail(){
    ...
    }catch(err){setLookup('שגיאת תקשורת לשירות האימות. נסו שוב.','err');console.error(err)}finally{authBusy(false)}
  }
  ...
  async function v85VerifyOtp(e){
    ...
    try{
      ...
      const ret=sessionStorage.getItem('tc_v85_return_after_auth')||'#account';sessionStorage.removeItem('tc_v85_return_after_auth');
