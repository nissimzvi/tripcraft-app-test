/* TripCraft Mobile V112 - unified account, save, mobile calendar and actions. */
(() => {
  'use strict';
  ...
  async function cloudRecords() {
    ...
    const readTable = async (table, source) => {
      try {
        ...
        console.warn(`V112: ${table} unavailable`, error?.message || error);
        return [];
      }
    };
  ...
  function renderAccountBody(list) {
    ...
    if (!box) return;
    if (!user) {
      box.innerHTML = `<div class="safe">${t('כדי לראות את הטיולים יש להיכנס עם אימייל וקוד אימות.', 'Sign in with your email and verification code to view your trips.')}</div>`;
      return;
    }
    const name = [user.firstName || user.first_name, user.lastName || user.last_name].filter(Boolean).join(' ') || t('החשבון שלי', 'My Account');
    const notice = Number(sessionStorage.getItem('tc_v112_saved_notice') || 0);
    ...
    box.innerHTML = `${saved}<section class="account-card tc-v112-account-card">
      <h3>${escapeHtml(name)}</h3>
      <p>${escapeHtml(user.email || '')}${user.phone ? ` · ${escapeHtml(user.phone)}` : ''}</p>
      <div class="cta"><button class="btn planner-color" id="tcV112NewTrip" type="button">${t('בניית טיול חדש', 'Build a New Trip')}</button></div>
    </section>
    ...
    if (sb && user) {
      for (const table of ['tripcraft_trips', 'trips']) {
        try {
          ...
          if (error) throw error;
        } catch (error) { console.warn(`V112: delete from ${table} skipped`, error?.message || error); }
      }
    }
  ...
  async function saveCurrentTrip(button) {
    ...
    try {
      ...
      sessionStorage.setItem('tc_v112_saved_notice', String(Date.now()));
      await renderAccount();
  ...
  function handleClick(event) {
    ...
    const open = event.target.closest?.('[data-v112-open]');
    if (open) {
      stop(event); open.disabled = true;
      openTrip(open.dataset.v112Open).catch(error => alert(t('לא ניתן לפתוח את הטיול: ', 'Could not open the trip: ') + error.message)).finally(() => { if (open.isConnected) open.disabled = false; });
      return;
    }
    const copy = event.target.closest?.('[data-v112-copy]');
    if (copy) { stop(event); copyText(actionUrl(copy.dataset.v112Copy), copy); return; }
    ...
    const remove = event.target.closest?.('[data-v112-delete]');
    if (remove) { stop(event); deleteTrip(remove.dataset.v112Delete); return; }
    if (event.target.closest?.('#tcV112NewTrip')) { stop(event); window.TripCraftV102?.resetNewTrip?.(); return; }
  ...
  function install() {
    ...
    ensureCalendar();
    enhancePlanner();
    window.TripCraftV112 = { VERSION, renderAccount, openTrip, saveCurrentTrip, deleteTrip };
    ...
    new MutationObserver(() => {
      if (queued) return;
      queued = true;
      requestAnimationFrame(() => { queued = false; ensureLogoutMenu(); enhancePlanner(); $('tcAccountBody')?.querySelectorAll('#tcLogout,#tcV102Logout').forEach(button => button.remove()); });
    ...
    document.documentElement.dataset.tripcraftBuild = 'V112-MOBILE';
  }
