/* TripCraft Mobile V115 - reliable save/update, AI changes and day details. */
(() => {
  'use strict';
  ...
  function enhancePlanner() {
    ...
    document.querySelectorAll('#plannerDraftDays > .day-row').forEach((row, index) => {
      ...
      if (button) button.dataset.v115OpenDay = String(index);
    });
    syncActionLabels();
  }
  ...
  function handleClick(event) {
    ...
    const dayButton = event.target.closest?.('[data-v115-open-day],#plannerDraftDays>.day-row .day-action>button:first-child');
  ...
  function install() {
    ...
    document.documentElement.dataset.tripcraftBuild = 'V115-MOBILE';
  }
