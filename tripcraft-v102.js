...
(() => {
  ...
  function route(){
    ...
    if(raw==='account')setTimeout(()=>window.TripCraftV112?.renderAccount?.()||renderAccount(),80);
  ...
  function install(){
    ...
    setTimeout(()=>{if(!window.TripCraftV112)window.tcRenderAccount=renderAccount;window.tcStartNewTrip=resetNewTrip;if(location.hash==='#account')(window.TripCraftV112?.renderAccount?.()||renderAccount())},600);
    ...
    window.addEventListener('hashchange',()=>{route();if(location.hash==='#account')setTimeout(()=>window.TripCraftV112?.renderAccount?.()||renderAccount(),700)});route();document.documentElement.dataset.tripcraftVersion=VERSION;
  }
