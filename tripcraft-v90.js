...
(() => {
  ...
  async function tcAuthRequest(endpoint,payload){
    ...
    try{
      const res=await fetch(url,{method:'POST',mode:'cors',cache:'no-store',credentials:'omit',signal:ctl.signal,headers:{'Content-Type':'application/json','apikey':c.supabaseAnonKey,'X-Client-Info':'tripcraft-v90'},body:JSON.stringify(payload||{})});
  ...
  function authReturn(hash){sessionStorage.setItem('tc_v90_return_after_auth',hash||'#account')}
  ...
  async function v85VerifyOtp(e){
    ...
    try{
      ...
      const ret=sessionStorage.getItem('tc_v90_return_after_auth')||'#account';sessionStorage.removeItem('tc_v90_return_after_auth');location.hash=ret;
