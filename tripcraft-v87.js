/* TripCraft V87 auth + cloud trips flow.
...
(() => {
  ...
  async function tcAuthRequest(endpoint,payload){
    ...
    try{
      const res=await fetch(url,{method:'POST',mode:'cors',cache:'no-store',credentials:'omit',signal:ctl.signal,headers:{'Content-Type':'application/json','apikey':c.supabaseAnonKey,'X-Client-Info':'tripcraft-v87'},body:JSON.stringify(payload||{})});
  ...
  function authReturn(hash){sessionStorage.setItem('tc_v87_return_after_auth',hash||'#account')}
  ...
  async function v85CheckEmail(){
    ...
    try{
      ...
      if((err?.status===400||err?.status===404)&&/Signups not allowed|User not found|not found|signup/i.test(msg)){
  ...
  async function v85VerifyOtp(e){
    ...
    const token=String(byId('tcOtpCode')?.value||'').replace(/\D/g,'');if(token.length!==6){byId('tcOtpStatus').textContent='נא להזין קוד בן 6 ספרות.';byId('tcOtpCode')?.focus();return}
    ...
    try{
      ...
      const ret=sessionStorage.getItem('tc_v87_return_after_auth')||'#account';sessionStorage.removeItem('tc_v87_return_after_auth');location.hash=ret;
